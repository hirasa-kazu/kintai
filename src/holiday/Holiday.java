...
public class Holiday{
	...
	private void init(int year){
		...
		if (year==2019){
			...
			set.add(LocalDate.of(2019, 5, 1));
			set.add(LocalDate.of(2019, 5, 2));
			set.add(LocalDate.of(2019, 10, 22));
		}
	...
	public static List<Map.Entry<LocalDate, String>> listHolidays(int year){
		...
		if (year==2019){
			map.put(LocalDate.of(2019, 4, 30), "国民の休日");
			map.put(LocalDate.of(2019, 5, 1), "平成の次の即位日");
			map.put(LocalDate.of(2019, 5, 2), "国民の休日");
			map.put(LocalDate.of(2019, 10, 22), "即位礼正殿の儀");
		}
	...
	public static int[] listHoliDays(int year, int month){
		...
		if (year < 2016 && month == 8) return new int[]{};  // since ver 2.0
	...
	class HealthSportsDayBundle extends HolidayBundle{
		...
		@Override
		public int getDay(){
			if (super.year==2020) return 24;
			if (super.year==2021) return 23;
			Calendar cal = Calendar.getInstance();
		...
		@Override
		public int getMonth(){
			return super.year==2020 || super.year==2021 ? 7 : 10;
		}
		@Override
		public String getDescription(){
			return super.year >= 2020 ? "スポーツの日" :"体育の日";
		}
	}
	// 文化の日
